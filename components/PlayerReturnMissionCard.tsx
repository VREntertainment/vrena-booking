...
const returnMissionCopy: Record<LanguageCode, ReturnMissionCopy> = {
  ...
  ko: {
    activeRun: '아레나 단골 연속 기록이 진행 중입니다.', arenaRegular: '연속 기록을 이어 아레나 단골을 잠금 해제하세요.', characterAlt: 'Wild West 카우보이', graceReady: '휴식 주 준비됨', graceUsed: '휴식 주 사용됨', joinSession: '세션 참가', keepMoving: '이번 주 한 번 방문하면 기록이 이어집니다.', reminderError: '알림을 설정하지 못했습니다. 다시 시도하세요.', reminderSet: '{date} 알림 설정됨', remindWeekend: '다음 주말에 알림', runSaved: '{game} 기록 저장됨', safeProgress: '한 번의 휴식 주가 진행 상황을 보호합니다.', streakBuilder: '다음 주에 돌아와 연속 플레이를 잠금 해제하세요.', weekFour: '4주 차', weekOne: '1주 차', weekTwo: '2주 차', weeks: '{current}/{target}주',
  },
  ja: {
    activeRun: 'アリーナ常連へのランが進行中です。', arenaRegular: 'ランを続けてアリーナ常連を解除しよう。', characterAlt: 'Wild Westのカウボーイ', graceReady: 'お休み週あり', graceUsed: 'お休み週使用済み', joinSession: 'セッションに参加', keepMoving: '今週あと1回来店するとランを維持できます。', reminderError: 'リマインダーを設定できませんでした。', reminderSet: '{date}に通知します', remindWeekend: '次の週末に通知', runSaved: '{game}の記録を保存しました', safeProgress: '1回のお休み週で進捗を守れます。', streakBuilder: '来週戻って連続プレイを解除しよう。', weekFour: '4週目', weekOne: '1週目', weekTwo: '2週目', weeks: '{current}/{target}週',
  },
  fr: {
    activeRun: 'Ta série Habitué de l’arène est active.', arenaRegular: 'Continue pour débloquer Habitué de l’arène.', characterAlt: 'Cow-boy de Wild West', graceReady: 'Semaine joker prête', graceUsed: 'Semaine joker utilisée', joinSession: 'Rejoindre une session', keepMoving: 'Une visite cette semaine maintient ta série.', reminderError: 'Impossible de programmer le rappel. Réessaie.', reminderSet: 'Rappel prévu le {date}', remindWeekend: 'Me rappeler le week-end prochain', runSaved: 'Partie {game} enregistrée', safeProgress: 'Une semaine joker protège ta progression.', streakBuilder: 'Reviens la semaine prochaine pour débloquer Série en cours.', weekFour: 'Semaine 4', weekOne: 'Semaine 1', weekTwo: 'Semaine 2', weeks: '{current} sur {target} semaines',
  },
  de: {
    activeRun: 'Deine Arena-Stammgast-Serie läuft.', arenaRegular: 'Bleib dran und schalte Arena-Stammgast frei.', characterAlt: 'Wild-West-Cowboy', graceReady: 'Pausenwoche bereit', graceUsed: 'Pausenwoche genutzt', joinSession: 'Session beitreten', keepMoving: 'Ein Besuch diese Woche hält deine Serie am Laufen.', reminderError: 'Erinnerung konnte nicht gesetzt werden.', reminderSet: 'Erinnerung für {date}', remindWeekend: 'Nächstes Wochenende erinnern', runSaved: '{game}-Runde gespeichert', safeProgress: 'Eine Pausenwoche schützt deinen Fortschritt.', streakBuilder: 'Komm nächste Woche zurück und schalte Serienmacher frei.', weekFour: 'Woche 4', weekOne: 'Woche 1', weekTwo: 'Woche 2', weeks: '{current} von {target} Wochen',
  },
  it: {
    activeRun: 'La tua serie Abituale dell’arena è attiva.', arenaRegular: 'Continua per sbloccare Abituale dell’arena.', characterAlt: 'Cowboy di Wild West', graceReady: 'Settimana jolly pronta', graceUsed: 'Settimana jolly usata', joinSession: 'Partecipa a una sessione', keepMoving: 'Una visita questa settimana mantiene attiva la serie.', reminderError: 'Impossibile impostare il promemoria. Riprova.', reminderSet: 'Promemoria per {date}', remindWeekend: 'Ricordamelo il prossimo weekend', runSaved: 'Partita {game} salvata', safeProgress: 'Una settimana jolly protegge i tuoi progressi.', streakBuilder: 'Torna la prossima settimana per sbloccare Serie in crescita.', weekFour: 'Settimana 4', weekOne: 'Settimana 1', weekTwo: 'Settimana 2', weeks: '{current} di {target} settimane',
  },
}
...
export default function PlayerReturnMissionCard({
  ...
  const missionMessage = mission.currentWeekVisits === 0
    ? copy.keepMoving
  ...
  const steps = useMemo(() => [
    ...
    { label: copy.weekTwo, state: current >= 2 ? 'complete' : 'current' },
    { label: mission.graceAvailable ? copy.graceReady : copy.graceUsed, state: 'grace' },
    { label: copy.weekFour, state: current >= 4 ? 'complete' : 'locked' },
  ] as const, [copy, current, mission.graceAvailable])
    ...
    <section className="return-mission-card" aria-labelledby="return-mission-title">
      <div className="return-mission-head">
        ...
        <div className="return-mission-copy">
          <h2 id="return-mission-title">{copy.runSaved.replace('{game}', gameTitle)}</h2>
          <p>{missionMessage}</p>
          <span><ShieldCheck aria-hidden="true" size={16} />{copy.safeProgress}</span>
        </div>
