...
function handleSessionDateChange(value: string) {
  ...
  async function shareTournamentResults(session: Session) {
    const { shareTournamentResultsImage } = await import('../lib/tournamentShareImage')
    const shareResult = await shareTournamentResultsImage({
      appUrl: DEFAULT_APP_URL,
      dateLabel: formatShortDate(session.date, language),
      fallbackPlayerLabel: text.player,
      participants: (session.session_participants ?? []).map((participant) => ({
        accuracyPercent: participant.accuracy_percent,
    ...
    if (shareResult === 'ready') setSharedKey(`results-${session.id}`)
  }
