...
type Participant = {
  ...
  avatar_url: string | null
  profiles?: Pick<Profile, 'phone' | 'email' | 'nickname' | 'avatar_url'> | null
}
...
type PricingRule = {
  id: string
  name: string
  day_type: 'any' | 'weekday' | 'weekend'
  start_time: string
  end_time: string
  price_per_player_20min: number
  minimum_total_20min: number
  min_players: number
  max_players: number
  active: boolean
}
...
export default function WidgetPage() {
  ...
  const [busyAdminSessionId, setBusyAdminSessionId] = useState('')
  const [blockDate, setBlockDate] = useState(localDateString())
  const [blockStartTime, setBlockStartTime] = useState('')
  const [blockEndTime, setBlockEndTime] = useState('')
  const [blockArenaCount, setBlockArenaCount] = useState(1)
  ...
  async function loadSessions() {
    const [sessionResult, blockedResult, pricingResult] = await Promise.all([
      supabase
        .from('sessions')
        .select('*, session_participants(id, profile_id, display_name, avatar_url, profiles(phone, email, nickname, avatar_url))')
        .neq('status', 'cancelled')
        .order('date', { ascending: true })
        .order('start_time', { ascending: true }),
      supabase.from('blocked_times').select('id, date, start_time, end_time, arenas_used, reason'),
    ...
    setPricingRules((pricingResult.data ?? []) as PricingRule[])
  }
  ...
  const priceEstimate = useMemo(() => {
    ...
    const startMinutes = timeToMinutes(sessionTime)
    const dayType = isWeekend(sessionDate) ? 'weekend' : 'weekday'
    const matchingRules = pricingRules
      .filter((rule) => rule.active)
      .filter((rule) => rule.day_type === 'any' || rule.day_type === dayType)
      .filter((rule) => sessionMaxPlayers >= rule.min_players && sessionMaxPlayers <= rule.max_players)
      .filter((rule) => {
      ...
      .sort((a, b) => {
        if (a.day_type === b.day_type) return b.minimum_total_20min - a.minimum_total_20min
        return a.day_type === 'any' ? 1 : -1
      })

    const rule = matchingRules[0]
    if (!rule) return null

    const durationBlocks = Math.ceil(sessionDuration / 20)
    const playerTotal = rule.price_per_player_20min * sessionMaxPlayers
    const minimumTotal = rule.minimum_total_20min * sessionArenaCount
    const total = Math.max(playerTotal, minimumTotal) * durationBlocks
  ...
  }, [pricingRules, sessionArenaCount, sessionDate, sessionDuration, sessionMaxPlayers, sessionTime])
  ...
  async function createSession() {
    ...
    const { data: created, error } = await supabase
      ...
      .insert({
        ...
        status: 'open',
        estimated_total: priceEstimate?.total || null,
        currency: priceEstimate?.currency || 'VND',
      })
      .select('id')
      .single()
   ...
   const notification = null
    ...
    setCreateStatus(
      sessionVisibility === 'private'
        ? `Private session created. Invite code: ${inviteCode}${notification?.ok ? ' Team notified.' : ''}`
        : `Session created.${notification?.ok ? ' Team notified.' : ''}`
    )
  ...
  async function cancelSession(sessionId: string) {
    ...
    setBusyAdminSessionId(sessionId)
    setAdminStatus('Cancelling session...')

    const { error } = await supabase.from('sessions').update({ status: 'cancelled' }).eq('id', sessionId)

    if (error) {
      setAdminStatus(error.message)
      setBusyAdminSessionId('')
      return
    }

    await loadSessions()
    setBusyAdminSessionId('')
    setAdminStatus('Session cancelled.')
  }
  ...
  async function createBlockedTime() {
    ...
    if (!blockDate || !blockStartTime || !blockEndTime) {
      setAdminStatus('Please enter date, start time, and end time.')
      return
    }

    if (timeToMinutes(blockEndTime) <= timeToMinutes(blockStartTime)) {
      setAdminStatus('End time must be after start time.')
      return
    }

    setIsBlockingTime(true)
    setAdminStatus('Blocking time...')

    const { error } = await supabase.from('blocked_times').insert({
      date: blockDate,
      start_time: `${blockStartTime}:00`,
      end_time: `${blockEndTime}:00`,
      arenas_used: blockArenaCount,
    ...
    setBlockStartTime('')
    setBlockEndTime('')
    setBlockArenaCount(1)
    setBlockReason('')
    await loadSessions()
    setIsBlockingTime(false)
    setAdminStatus('Time blocked.')
  }
  ...
  return (
    <div className="app">
      ...
      <main>
                    ...
                    <div className="session-top">
                      <div>
                        ...
                        <div className="row-meta">
                          ...
                          {session.estimated_total ? <span>{formatCurrency(session.estimated_total, session.currency || 'VND')}</span> : null}
                        </div>
                      </div>
          ...
          <section className="section">
            ...
            <div className="price-summary">
              <div>
                ...
                <strong>
                  {priceEstimate ? formatCurrency(priceEstimate.total, priceEstimate.currency) : 'Choose a date and time'}
                </strong>
              </div>
              <p>{priceEstimate ? `Based on ${priceEstimate.ruleName}. Final confirmation by VRena team.` : 'Pricing rules are loaded from Supabase.'}</p>
            </div>
        ...
        {activeView === 'admin' && (
          <section className="section">
            <h2>Admin</h2>
            {profile?.role !== 'admin' ? (
              <p className="notice">Admin controls are only visible for VRena team accounts.</p>
            ) : (
              <>
                <p className="muted">Manage sessions, see player contacts, and block arena time.</p>
                ...
                <div className="admin-panel">
                  ...
                  <div className="form-grid">
                    ...
                    <div>
                      <label>Arenas</label>
                      <select value={blockArenaCount} onChange={(event) => setBlockArenaCount(Number(event.target.value))}>
                        <option value={1}>1 arena</option>
                    ...
                    <div className="full">
                      <label>Reason</label>
                      <input value={blockReason} onChange={(event) => setBlockReason(event.target.value)} placeholder="Maintenance, staff event, private booking..." />
                    </div>
                  </div>
                  <button className={isBlockingTime ? 'primary loading create-button' : 'primary create-button'} disabled={isBlockingTime} onClick={createBlockedTime}>
                    {isBlockingTime ? 'Blocking...' : 'Block Time'}
                  </button>
                </div>
                    ...
                    {sessions.map((session) => (
                      <article className="session" key={session.id}>
                        <div className="session-top">
                          ...
                          <button
                            className={busyAdminSessionId === session.id ? 'danger loading' : 'danger'}
                            disabled={busyAdminSessionId === session.id}
                            onClick={() => cancelSession(session.id)}
                          >
                            {busyAdminSessionId === session.id ? 'Cancelling...' : 'Cancel'}
                          </button>
                        </div>
                        ...
                        <div className="contact-list">
                          {(session.session_participants ?? []).map((participant) => (
                            <div key={participant.id}>
                              <strong>{participant.display_name || participant.profiles?.nickname || 'Player'}</strong>
                              <span>{participant.profiles?.phone || 'No phone'}</span>
                              <span>{participant.profiles?.email || 'No email'}</span>
                            </div>
                          ))}
                        </div>
                      </article>
                ...
                {adminStatus && <p className="notice">{adminStatus}</p>}
              </>
            )}
          </section>
        )}
      ...
      <style jsx>{`
        ...
        .contact-list div {
          display: grid;
          grid-template-columns: minmax(140px, 1fr) minmax(120px, 1fr) minmax(160px, 1fr);
          gap: 8px;
          padding: 8px;
          border-radius: 6px;
          background: #f0f4f6;
