...
test('prices each player for every 45-minute session', () => {
  ...
  assert.equal(pricing.totalPrice, 1_872_000)
})
...
test('caps billed players and bases discounts on total paid player-slots', () => {
  for (const [capacity, arenas, guests, billed] of [
    ...
    for (const duration of [45,90,135,180]) {
      ...
      const rate=slots>=9&&slots<=16?0.15:slots>=5&&slots<=8?0.1:0
  ...
  assert.equal(calculateTicketPricing(220000,8,90,45,4,1).totalPrice,1584000)
  assert.equal(calculateTicketPricing(220000,8,135,45,4,1).totalPrice,2244000)
})
