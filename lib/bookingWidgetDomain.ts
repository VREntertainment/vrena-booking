...
import { ARENA_COUNT, localDateString } from './booking/availability.ts'
import { matchWinnerFromSeries } from './booking/tournamentRules.ts'
import {
  countries,
  dateLocales,
  games,
...
import { individualTicketUnitPrice as resolveIndividualTicketUnitPrice, ticketPriceBlockMinutesForDate, type TicketPricingVenue } from './ticketTariffs'
...
export function ticketRequiredSlots(
  ...
  return Math.max(1, Math.ceil(Math.max(1, players) / (ticketArenaCapacityForVenue(venue) * ticketArenaCountForPlayers(arenaCount))))
}
...
export function ticketMinimumDurationBlocks(
  ...
  return minimumTicketDurationMinutes(
    players,
    priceBlockMinutes,
    ticketArenaCapacityForVenue(venue),
...
export function ticketPricingSummary(
  ...
  const arenaCapacity = ticketArenaCapacityForVenue(venue)
  const requiredSlots = ticketMinimumDurationBlocks(players, selectedArenaCount, priceBlockMinutes, venue)
...
export function ticketDurationForPlayers(
  ...
  const priceBlockMinutes = ticketPriceBlockMinutesForDate(dateValue)
  return Math.max(selectedTicketService(ticketType).duration, ticketMinimumDurationBlocks(players, arenaCount, priceBlockMinutes, venue) * priceBlockMinutes)
}
...
export { ARENA_COUNT,arenasUsedBySession,CLOSE_MINUTES,localDateString,minutesToTime,OPEN_MINUTES,rangesOverlap,TIME_STEP_MINUTES,timeToMinutes } from './booking/availability.ts'

export { clampTicketLoyaltyRedemption } from './booking/checkout.ts'

export { buildKnockoutRows,eligibleTournamentParticipants,hasDuplicateMatchPlayers,isPaidParticipant,knockoutStageForCount,matchLoser,matchWinnerFromSeries,qualificationCount,shuffleItems,winsNeeded } from './booking/tournamentRules.ts'
