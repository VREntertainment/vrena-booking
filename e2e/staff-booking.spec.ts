...
test('staff booking: exact event duration, outside hours, contact and live conflict check', async ({ page }, testInfo) => {
  ...
  try {
    ...
    await page.getByRole('textbox', { name: 'End time: type a specific time', exact: true }).fill('12:20')
    await page.getByRole('textbox', { name: 'End time: type a specific time', exact: true }).press('Enter')
...
test('staff booking: annotated form, VAT notes, two arenas and JPEG sharing', async ({ page }, testInfo) => {
  test.skip(testInfo.project.name !== 'chromium', 'Desktop and mobile covered in one flow.')
  if (process.env.SUPABASE_URL !== 'http://127.0.0.1:56431') throw new Error('Local services only')
  const admin = createClient(process.env.SUPABASE_URL, process.env.SUPABASE_SERVICE_ROLE_KEY!, { auth: { persistSession: false } })
  let sessionId: string | undefined
  const errors: string[] = []
  page.on('pageerror', (error) => errors.push(error.message))
  try {
    ...
    await page.setViewportSize({ width: 1142, height: 894 })
    await page.getByRole('checkbox', { name: 'Guest booking', exact: true }).check()
    await expect(page.getByRole('textbox', { name: 'End time', exact: true })).toHaveAttribute('readonly', '')
    const notes = page.getByRole('textbox', { name: 'Booking notes / special requests', exact: true })
    await notes.fill('Keep this request')
    const event = page.getByRole('checkbox', { name: 'Event / corporate', exact: true })
    await event.check()
    await expect(notes).toHaveValue('Keep this request\n\nDetails for VAT invoice:\nCompany name:\nCompany address:\nCompany VAT number:')
    await event.uncheck()
    await event.check()
    expect((await notes.inputValue()).match(/Details for VAT invoice/g)).toHaveLength(1)
    await expect(page.getByRole('spinbutton', { name: 'Time booked (minutes)', exact: true })).toHaveCount(0)
    await expect(page.getByText('Opening hours:', { exact: false })).toHaveCount(0)
    await page.getByRole('combobox', { name: 'Game', exact: true }).selectOption('none')
    await page.getByRole('combobox', { name: 'Number of arenas', exact: true }).selectOption('2')
    await page.getByLabel('Booking date', { exact: true }).fill(futureDate(227))
    await page.getByRole('checkbox', { name: 'Allow booking outside opening hours', exact: true }).check()
    await page.getByRole('button', { name: 'Booking time', exact: true }).click()
    await page.getByRole('textbox', { name: 'Booking time: type a specific time', exact: true }).fill('07:13')
    await page.getByRole('textbox', { name: 'Booking time: type a specific time', exact: true }).press('Enter')
    await page.getByRole('button', { name: 'End time', exact: true }).click()
    await page.getByRole('textbox', { name: 'End time: type a specific time', exact: true }).fill('08:40')
    await page.getByRole('textbox', { name: 'End time: type a specific time', exact: true }).press('Enter')
    await page.getByRole('checkbox', { name: 'Override total', exact: true }).check()
    await page.getByRole('spinbutton', { name: 'Final total (VND)', exact: true }).fill('1080000')
    await page.getByRole('textbox', { name: 'Reason for price override', exact: true }).fill('Corporate package')
    const summary = page.locator('.staff-price-lines')
    await expect(summary).toContainText('87 min')
    await expect(summary.locator('span').filter({ hasText: /^Total before VAT$/ }).locator('+ strong')).toHaveText('1.000.000 đ')
    await expect(summary.locator('span').filter({ hasText: /^Total \(VAT included\)$/ }).locator('+ strong')).toHaveText('1.080.000 đ')
    const share = page.getByRole('button', { name: 'Share', exact: true })
    await expect(share).toBeEnabled({ timeout: 20000 })
    await page.evaluate(() => { Object.defineProperty(navigator, 'canShare', { configurable: true, value: () => false }) })
    const download = page.waitForEvent('download')
    await share.click()
    const jpeg = await download
    expect(jpeg.suggestedFilename()).toMatch(/\.jpg$/)
    await jpeg.saveAs('/tmp/staff-summary-share.jpg')
    await page.evaluate(() => {
      Object.defineProperty(navigator, 'canShare', { configurable: true, value: () => true })
      Object.defineProperty(navigator, 'share', { configurable: true, value: async (data: ShareData) => {
        const file = data.files![0]
        document.body.dataset.sharedFile = `${file.type}:${file.size}:${file.name}`
      } })
    })
    await share.click()
    await expect(page.locator('body')).toHaveAttribute('data-shared-file', /^image\/jpeg:[1-9][0-9]*:VRena-booking-/)
    await page.screenshot({ path: '/tmp/staff-annotations-desktop.png', fullPage: true })
    await page.setViewportSize({ width: 375, height: 667 })
    expect(await page.evaluate(() => document.documentElement.scrollWidth <= innerWidth + 1)).toBe(true)
    await page.screenshot({ path: '/tmp/staff-annotations-mobile.png', fullPage: true })
    const response = page.waitForResponse((res) => res.url().endsWith('/rpc/staff_create_booking'))
    await page.getByRole('button', { name: 'Confirm booking', exact: true }).click()
    const created = await (await response).json()
    sessionId = created.session_id
    expect(sessionId).toBeTruthy()
    const saved = await admin.from('sessions').select('arena_count,duration_minutes,game_options,notes').eq('id', sessionId!).single()
    expect(saved.data).toMatchObject({ arena_count: 2, duration_minutes: 87, game_options: [] })
    expect(saved.data!.notes).toContain('Company VAT number:')
    expect(errors).toEqual([])
  } finally {
    if (sessionId) await admin.from('sessions').delete().eq('id', sessionId)
  }
...
test('staff booking: discount valid-until saves, reopens and expires inclusively', async ({ page }, testInfo) => {
  test.skip(testInfo.project.name !== 'chromium', 'Local write fixture only.')
  if (process.env.SUPABASE_URL !== 'http://127.0.0.1:56431') throw new Error('Local services only')
  const admin = createClient(process.env.SUPABASE_URL, process.env.SUPABASE_SERVICE_ROLE_KEY!, { auth: { persistSession: false } })
  const name = `Expiry QA ${Date.now()}`
  try {
    await loginAsAdmin(page)
    await openAdmin(page)
    await page.getByRole('tab', { name: 'Offers', exact: true }).click()
    await page.getByRole('textbox', { name: 'Name', exact: true }).fill(name)
    await page.getByLabel('Discount valid from', { exact: true }).fill(futureDate(240))
    await page.getByLabel('Discount valid until', { exact: true }).fill(futureDate(241))
    await page.getByRole('button', { name: 'Save discount', exact: true }).click()
    const item = page.locator('.staff-list-item').filter({ hasText: name })
    await item.click()
    await expect(page.getByLabel('Discount valid until', { exact: true })).toHaveValue(futureDate(241))
    const saved = await admin.from('staff_discount_rules').select('valid_until').eq('name', name).single()
    expect(saved.data?.valid_until).toBe(futureDate(241))
    await page.getByRole('tab', { name: 'New Booking', exact: true }).click()
    await page.getByLabel('Booking date', { exact: true }).fill(futureDate(241))
    const options = page.getByRole('combobox', { name: 'Discount / voucher', exact: true }).locator('option')
    await expect(options).toContainText([name])
    await page.getByLabel('Booking date', { exact: true }).fill(futureDate(242))
    await expect(options.filter({ hasText: name })).toHaveCount(0)
    await page.getByRole('tab', { name: 'Offers', exact: true }).click()
    await item.click()
    await page.getByLabel('Discount valid until', { exact: true }).fill('')
    await page.getByRole('button', { name: 'Save discount', exact: true }).click()
    await item.click()
    await expect(page.getByLabel('Discount valid until', { exact: true })).toHaveValue('')
    const cleared = await admin.from('staff_discount_rules').select('valid_until').eq('name', name).single()
    expect(cleared.data?.valid_until).toBeNull()
  } finally {
    await admin.from('staff_discount_rules').delete().eq('name', name)
  }
})

test('staff booking: session count follows the shared session length', async ({ page }, testInfo) => {
  test.skip(testInfo.project.name !== 'chromium', 'Use the isolated service once.')
  await loginAsAdmin(page)
  await openAdmin(page)
  await page.getByLabel('Booking date', { exact: true }).fill(futureDate(185))
  const count = page.getByRole('spinbutton', { name: 'Number of sessions', exact: true })
  const end = page.getByLabel('End time', { exact: true })
  await expect(count).toHaveValue('1')
  await expect(end).toHaveValue('09:45')
  await count.fill('2')
  await expect(end).toHaveValue('10:30')
  await expect(page.locator('.staff-price-lines')).toContainText('90 min')
  await page.route('**/rest/v1/rpc/staff_booking_session_minutes', route => route.fulfill({ status: 200, contentType: 'application/json', body: '60' }))
  await page.getByLabel('Booking date', { exact: true }).fill(futureDate(186))
  await expect(end).toHaveValue('11:00')
  await expect(page.locator('.staff-price-lines')).toContainText('120 min')
  await page.getByRole('checkbox', { name: 'Event / corporate', exact: true }).check()
  await page.getByRole('button', { name: 'End time', exact: true }).click()
  await page.getByRole('textbox', { name: 'End time: type a specific time', exact: true }).fill('12:17')
  await page.getByRole('textbox', { name: 'End time: type a specific time', exact: true }).press('Enter')
  await count.fill('3')
  await expect(page.getByRole('button', { name: 'End time', exact: true })).toHaveText('12:17')
})

test('staff booking: customer loyalty defaults on and opt-out survives pricing changes', async ({ page }, testInfo) => {
  test.skip(testInfo.project.name !== 'chromium', 'Use the isolated service once.')
  await loginAsAdmin(page)
  ...
  const withPoints = Number((await total.innerText()).replace(/[^0-9]/g, ''))
  await toggle.uncheck()
  await expect(page.locator('.ticket-discount-line')).toHaveCount(0)
  await expect.poll(async () => Number((await total.innerText()).replace(/[^0-9]/g, ''))).toBe(withPoints + 50000)
  await expect(page.locator('.ticket-reserved-line .ticket-average-player-price')).toBeVisible()
  await page.locator('#ticket-player-count').selectOption('8')
