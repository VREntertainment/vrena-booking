...
('QA Group 5-8','percentage',5,5,8,'all','2020-01-01',true),
('QA Group 9-16','percentage',5,9,16,'all','2020-01-01',true),
('QA Birthday','percentage',5,null,null,'birthday','2020-01-01',true);
...
select is((select (result->>'total')::int from discount_results where method='5'),1140000,'Five players receive the configured 5 percent group discount');
select is((select (result->>'total')::int from discount_results where method='9'),2052000,'Nine players receive the configured 5 percent group discount');
select is((select (result->>'total')::int from discount_results where method='16'),3648000,'Sixteen players retain the configured 5 percent group discount');
...
select is((select (result->>'total')::int from discount_results where method='birthday'),912000,'Staff can select the configured birthday offer');
...
select is((select price_override_original_total from public.staff_orders where id=(select (result->>'order_id')::uuid from discount_results where method='override')),1140000,'Original total includes the automatic group discount');
