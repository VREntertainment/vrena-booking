...
as $$
  ...
  select
    visible_clubs.id,
    visible_clubs.owner_id,
    visible_clubs.name,
    visible_clubs.motto,
    visible_clubs.description,
    visible_clubs.banner_url,
    visible_clubs.theme_color,
    visible_clubs.default_language,
    visible_clubs.ranking_criterion,
    visible_clubs.visibility,
    visible_clubs.pin_code,
    visible_clubs.member_count,
    visible_clubs.created_at,
    coalesce((
      select jsonb_agg(
        jsonb_build_object(
          'id', cm.id,
      ...
      where cm.club_id = visible_clubs.id
        and cm.deleted_at is null
        and (
          (cm.status = 'approved' and visible_clubs.can_view_members)
          or cm.profile_id = (select auth.uid())
          or visible_clubs.can_manage_members
        )
    ), '[]'::jsonb) as club_members
  from visible_clubs
  order by visible_clubs.created_at desc nulls last, visible_clubs.name asc;
$$;
