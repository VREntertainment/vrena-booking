-- Remove accidental E2E test-user access from production.
--
-- This migration drops the local/staging E2E helper functions if they ever
-- reached production and disables matching E2E auth users without hard-deleting
-- auth.users rows. Hard-deleting auth.users would cascade through profiles and
-- could remove app data, so this neutralizes auth access and soft-deletes the
-- matching profiles instead.
...
begin
  ...
  raise notice 'E2E users selected for cleanup: %', coalesce(v_emails::text, '{}');
